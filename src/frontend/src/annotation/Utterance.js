...
export default function Utterance({ url, utterance, utteranceContext, audioPlaying, setAudioPlaying }) {
  ...
  const renderRow = (rowUtterance, isHighlighted = false) => (
    <>
      ...
      <tr key={`${rowUtterance.start}-${rowUtterance.end}-${i++}`} className={`table-sm ${isHighlighted ? '' : 'text-muted'}`}>
        <td>{secondsToHms(rowUtterance.start)}</td>
        <td>{secondsToHms(rowUtterance.end)}</td>
        <td>{rowUtterance.speaker}</td>
        <td>
          <Button onClick={() => { seekToTime(rowUtterance.start); setAudioPlaying(true); console.log(rowUtterance.start) }} variant="outline-success" size="sm">
  ...
  return (
    <Card className='mb-3' style={{ minHeight: '25rem', backgroundColor: "#f8f9fa" }}>
